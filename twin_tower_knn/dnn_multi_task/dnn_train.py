...
tf.app.flags.DEFINE_integer("batch_size", 16, "Batch size for training/evaluating.")
...
tf.app.flags.DEFINE_integer("batch_save", 5, "save model how many batch")
            ...
            for epoch in range(curr_epoch,FLAGS.num_epochs):
                ...
                while train_example_num < train_sample_num:
                    try :
                        ...
                        query, cid, cid_neg, product, product_neg = train_batch_data
                        if len(query)!=FLAGS.batch_size:
                            continue
                        feed_dict = {model.query: query, model.cid: cid, model.product: product, model.cid_neg: cid_neg, model.product_neg: product_neg,  model.dropout_keep_prob: 0.5}
...
def assign_pretrained_word_embedding(sess, vocabulary_index2word, vocab_size, w2v_model):
    print("using pre-trained word emebedding.started.word2vec_model_path:",w2v_model)
    f = open(w2v_model)
    word2vec_dict = {}
    for line in f:
