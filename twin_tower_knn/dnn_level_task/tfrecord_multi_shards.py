    ...
    def _sample_to_tfrecord(shard_index):
        ...
        with open(FLAGS.input_examples_file) as f:
            for i, line in enumerate(f):
                ...
                fl_pos_brd = example.feature_lists.feature_list["pos_brd"]
                fl_neg_brd = example.feature_lists.feature_list["neg_brd"]
                fl_pos_pdt = example.feature_lists.feature_list["pos_pdt"]
                fl_neg_pdt = example.feature_lists.feature_list["neg_pdt"]
                fl_label = example.feature_lists.feature_list["labels"]
                ...
                if len(tokens) == 8:
                    ...
                    pos_pdt = tokens[6].strip().split(',')[:2]
                    neg_pdt = tokens[7].strip().split(',')[:2]
                    while len(terms) < 8:
                        terms.append('<PAD>')
                    while len(pos_cid) < 6:
                        pos_cid.append('<PAD>')
