...
class DNN:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, vocab_size, embed_size, hidden_size, is_training, tag_flag=True, initializer=initializers.xavier_initializer(), clip_gradients=5.0):
        ...
        self.cid3= tf.placeholder(tf.int32, [None, None], name="cid3")
        self.cid3_mask = tf.cast(tf.not_equal(self.cid3, 0), tf.float32)
        ...
        self.cid3_neg= tf.placeholder(tf.int32, [None, None], name="cid3_neg")
        self.cid3_neg_mask = tf.cast(tf.not_equal(self.cid3_neg, 0), tf.float32)
        ...
        self.brand_neg= tf.placeholder(tf.int32, [None, None], name="brand_neg")
        self.brand_neg_mask = tf.cast(tf.not_equal(self.brand_neg, 0), tf.float32)
    ...
    def query_dnn(self, embedds, mask ):
        ...
        with tf.variable_scope("query_hidden", reuse=tf.AUTO_REUSE):
    ...
    def cid_dnn(self, embedds, mask ):
        terms_mask = tf.expand_dims(mask, -1)
        input_feature = tf.multiply(terms_mask, embedds)
        sentence_feature = tf.reduce_sum(input_feature, axis=1)
        with tf.variable_scope("cid_hidden", reuse=tf.AUTO_REUSE):
            h1 = tf.layers.dense(sentence_feature, 512, activation=tf.nn.tanh)
            h2 = tf.layers.dense(h1, 256, activation=tf.nn.tanh)
        #h_drop = tf.nn.dropout(h2, keep_prob=self.dropout_keep_prob)
        return h2   
    
    def cid3_dnn(self, embedds, mask ):
        terms_mask = tf.expand_dims(mask, -1)
        input_feature = tf.multiply(terms_mask, embedds)
        sentence_feature = tf.reduce_sum(input_feature, axis=1)
        # concat cid embedding
        sentence_feature = tf.concat(axis=1, values=[sentence_feature, self.cid_f])
        with tf.variable_scope("cid3_hidden", reuse=tf.AUTO_REUSE):
            h1 = tf.layers.dense(sentence_feature, 512, activation=tf.nn.tanh)
            h2 = tf.layers.dense(h1, 256, activation=tf.nn.tanh)
        #h_drop = tf.nn.dropout(h2, keep_prob=self.dropout_keep_prob)
        return h2   
    
    def product_dnn(self, embedds, mask ):
        terms_mask = tf.expand_dims(mask, -1)
        input_feature = tf.multiply(terms_mask, embedds)
        sentence_feature = tf.reduce_sum(input_feature, axis=1)

        # concat cid embedding
        sentence_feature = tf.concat(axis=1, values=[sentence_feature, self.cid3_f])
        with tf.variable_scope("product_hidden", reuse=tf.AUTO_REUSE):
            h1 = tf.layers.dense(sentence_feature, 512, activation=tf.nn.tanh)
            h2 = tf.layers.dense(h1, 256, activation=tf.nn.tanh)
        #h_drop = tf.nn.dropout(h2, keep_prob=self.dropout_keep_prob)
        return h2   
    ...
    def inference_dnn(self):
        ...
        self.embedded_cid3 = tf.nn.embedding_lookup(self.Embedding,self.cid3)
        self.embedded_cid3_neg = tf.nn.embedding_lookup(self.Embedding,self.cid3_neg)
        ...
        # get query emb
        self.query_f = self.query_dnn(self.embedded_query, self.query_mask)

        # get cid emb
        self.cid_f = self.cid_dnn(self.embedded_cid, self.cid_mask)
        self.cid_n_f = self.cid_dnn(self.embedded_cid_neg, self.cid_neg_mask)
        
        # get cid3 emb
        self.cid3_f = self.cid3_dnn(self.embedded_cid3, self.cid3_mask)
        self.cid3_n_f = self.cid3_dnn(self.embedded_cid3_neg, self.cid3_neg_mask)

        # get product emb
        self.product_f = self.product_dnn(self.embedded_product, self.product_mask)
        self.product_n_f = self.product_dnn(self.embedded_product_neg, self.product_neg_mask)
        # normalize 
        ...
        self.cid3_f = tf.nn.l2_normalize(self.cid3_f, dim=1)
        self.cid3_n_f = tf.nn.l2_normalize(self.cid3_n_f, dim=1)
        ...
        query_cid3 = tf.multiply(self.query_f,self.cid3_f)
        self.query_cid3 = tf.reduce_sum(query_cid3,axis=1)

        query_n_cid3 = tf.multiply(self.query_f, self.cid3_n_f)
        self.query_n_cid3 = tf.reduce_sum(query_n_cid3, axis=1)
        ...
        cid3_loss=self.query_n_cid3 - self.query_cid3 + 0.5 
        ...
        cid3_p_weight = tf.pow(1-self.query_cid3, 2)
        cid3_n_weight = tf.pow(tf.maximum(self.query_n_cid3,0), 2)
        ...
        cid3_weight = cid3_p_weight + cid3_n_weight
        product_weight = product_p_weight + product_n_weight 
        
        with tf.name_scope("output"):
            cid_loss = tf.maximum(cid_loss, tf.zeros_like(cid_loss))
            cid3_loss = tf.maximum(cid3_loss, tf.zeros_like(cid3_loss))
            product_loss = tf.maximum(product_loss, tf.zeros_like(product_loss))
           
            # weighted  
            logits = cid_weight*cid_loss + cid3_weight*cid3_loss + product_weight*product_loss
            return logits
    ...
    def instantiate_dnn(self):
        with tf.name_scope("embedding_projection"):
            ...
            #self.W_intention = tf.get_variable("W_intention", shape=[256, self.num_classes], initializer=self.initializer)
            #self.b_intention = tf.get_variable("b_intention", shape=[self.num_classes])
            #self.attention = tf.get_variable("attention", shape= [3], initializer=tf.constant_initializer([1,1,1]))
