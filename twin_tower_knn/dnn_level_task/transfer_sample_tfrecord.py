...
def sample_to_tfrecord(sample_file, tfrecord_file, term_index):
    ...
    with open(sample_file) as f:
        for i, line in enumerate(f):
            ...
            fl_pos_cid3 = example.feature_lists.feature_list["pos_cid3"]
            fl_neg_cid3 = example.feature_lists.feature_list["neg_cid3"]
            ...
            if tokens[0].strip()!='' and tokens[0].strip()!='NULL':
                ...
                pos_cid = tokens[1].strip().split(',')
                neg_cid = tokens[2].strip().split(',')
                pos_cid3 = tokens[3].strip().split(',')
                neg_cid3 = tokens[4].strip().split(',')
                pos_pdt = tokens[5].strip().split(',')
                neg_pdt = tokens[6].strip().split(',')
                while len(terms) <8:
                    terms.append('<PAD>')
                for x in [pos_cid, neg_cid]:
                    while len(x) <4:
                        x.append('<PAD>')
                for x in [pos_cid3, neg_cid3]:
                    while len(x) <6:
                        x.append('<PAD>')
                ...
                for term in pos_cid3:
                    if term in term_index:
                        fl_pos_cid3.feature.add().int64_list.value.append(term_index[term])
                    else:
                        fl_pos_cid3.feature.add().int64_list.value.append(1)
                for term in neg_cid3:
                    if term in term_index:
                        fl_neg_cid3.feature.add().int64_list.value.append(term_index[term])
                    else:
                        fl_neg_cid3.feature.add().int64_list.value.append(1)
                for term in pos_pdt:
                    if term in term_index:
