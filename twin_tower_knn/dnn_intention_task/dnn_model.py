...
class DNN:
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, vocab_size, embed_size, hidden_size, is_training, tag_flag=True, initializer=initializers.xavier_initializer(), clip_gradients=5.0):
        ...
        intention_index = tf.argmax(self.intention_logits,axis=-1)
        self.intention_acc = tf.reduce_mean(tf.cast(tf.equal(tf.cast(intention_index, tf.int32), self.intention_label),tf.float32))
    ...
    def cid_dnn(self, embedds, mask ):
        ...
        label_embedding = tf.gather(self.W_intention, self.intention_label, axis=1)
        label_embedding = tf.reshape(label_embedding, [self.batch_size, 256])
        sentence_feature = tf.concat(axis=1, values=[sentence_feature, label_embedding]) 
    ...
    def inference_dnn(self):
        ...
        cid_loss=self.query_n_cid - self.query_cid + 0.7 
        product_loss=self.query_n_product - self.query_product + 0.7 
        ...
        with tf.name_scope("multi_class"):
            self.intention_logits = tf.matmul(self.query_f, self.W_intention) + self.b_intention
            log_probs = tf.nn.log_softmax(self.intention_logits, axis=-1)
            one_hot_labels = tf.one_hot(self.intention_label, depth=self.num_classes, dtype=tf.float32)
            per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1)
            self.intention_loss = tf.reduce_mean(per_example_loss) 
    ...
    def loss(self, l2_lambda=0.0001):
        with tf.name_scope("loss"):
            ...
            pairwise_loss = loss + l2_loss + self.intention_loss 
