...
from utils import result_to_json, show_result
...
class Model(object):
    ...
    def evaluate_bach(self, sess, data_manager, id_to_tag, result_path):
        ...
        for batch in data_manager.iter_batch():
            ...
            batch_paths = self.decode(scores, lengths, trans)
            i = 0
            for result in batch_paths:
                tags = [id_to_tag[idx] for idx in result]
                string = show_result(batch[0][i], tags)
                i+=1
                f.write(string + '\n')
        f.close()
    ...
    def evaluate_line(self, sess, inputs, id_to_tag):
        ...
        return show_result(inputs[0][0], tags)
