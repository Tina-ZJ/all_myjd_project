...
import numpy as np
from HAN_model_v4_geometric_loss import HierarchicalAttention
...
tf.app.flags.DEFINE_integer("num_classes_first",500,"number of cid")
tf.app.flags.DEFINE_integer("num_classes_second",5926,"number of cid2")
tf.app.flags.DEFINE_integer("num_classes_product",54949,"number of product")
tf.app.flags.DEFINE_integer("num_classes_brand",76545,"number of brand")
tf.app.flags.DEFINE_integer("num_classes_third",5926,"number of cid3")
...
tf.app.flags.DEFINE_string("cid3_product_relation","data/cid3_product.weight",'path of cid2 and cid3 relation')
...
tf.app.flags.DEFINE_boolean("relation_flag",False,"whether to run training")
...
tf.app.flags.DEFINE_string("output_dir","./output_v4/checkpoint",'save model path')
...
def model_fn_builder(vocab_size, cid2_weight, cid3_weight, brand_weight, relation_matrix, product_matrix, brand_matrix):
    def model_fn(features, labels, mode, params):
        ...
        input_tag = tf.ones(tf.shape(input_x), dtype=tf.int64)
        model = HierarchicalAttention( FLAGS.keep_prob, cid2_weight, cid3_weight, brand_weight, relation_matrix, product_matrix, brand_matrix, input_x, input_y_first, input_y_second, input_product, input_brand, input_tag, 
                                       ...
                                       FLAGS.num_classes_product+1, FLAGS.num_classes_brand+1, FLAGS.num_tags, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps,
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            output_spec = tf.estimator.EstimatorSpec(
                    ...
                    predictions={"predictions": model.predictions,
                                 "tag_predictions": model.tag_predictions},
                                  export_outputs={SIGNATURE_NAME: PredictOutput({"predictions": model.predictions,
                                                                                 "cid2_predictions": model.first_scores,
                                                                                 "tag_predictions": model.tag_predictions,
                                                                                 "product_predictions": model.product_predictions,
                                                                                 "attentions": model.first_visual,
                                                                                 "embedding": model.h_drop,
...
if __name__=='__main__':
    ...
    if FLAGS.relation_flag:
        relation_matrix = get_relation(FLAGS.cid2_cid3_relation)
    ...
    model_fn = model_fn_builder(
            vocab_size=vocab_size+2, cid2_weight=1.0, cid3_weight=1.0, brand_weight=1.0, relation_matrix=relation_matrix, product_matrix=1.0, brand_matrix=1.0)
