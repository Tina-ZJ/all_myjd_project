...
if __name__=='__main__':
    ...
    t_job = te.TensorFlow(entry_point='/media/cfs/zhangjun386/Sugg_rank/train_click.py',\
                        train_gpu_count=1,
                        hyperparameters={'train_sample_num':FLAGS.train_sample_num, 'dev_sample_num':FLAGS.dev_sample_num}, \
                        )
    t_job.fit(base_job_name='suggest-rank', group='ea-search-rank-ht', cluster='sr-cluster', node_labels=['mjq','search','search-lf','train','search-ht'], cpu=12, memory=10)
