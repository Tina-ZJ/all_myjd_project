...
tf.app.flags.DEFINE_string("output_dir","./output_large/checkpoint",'save model path')
tf.app.flags.DEFINE_string("init_checkpoint",None,'init  model parameter from pretrain model')
tf.app.flags.DEFINE_string("bert_config_file",'./bert_config_large.json', 'bert config file')
tf.app.flags.DEFINE_integer("save_checkpoints_steps",10000, "how many steps to make estimator call")


#command = "hadoop fs -ls hdfs://ns1013/user/recsys/suggest/app.db/qp_common_file/suggest_rank/2022-01-12/bert/v4/ | grep part | awk '{print $NF}' 2>/dev/null"
command = "hadoop fs -ls hdfs://ns1013/user/recsys/suggest/app.db/qp_common_file/suggest_rank/2022-03-08/bert/v6/ | grep part | awk '{print $NF}' 2>/dev/null"
...
def lambda_rank(score, labels, group_id, sigma, lr):
    ...
    loss = tf.cond(tf.equal(num_pairs, 0), lambda: 0., lambda: tf.reduce_sum(loss * mask) / num_pairs)
...
def model_fn_builder(init_checkpoint, term_size, bert_config):
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            logits, predictions = inference(model)
            
            # lambda rank
            #loss, num_pairs, train_op = lambda_rank(score, relative_label, group_id,  FLAGS.sigma, FLAGS.lr ) 
