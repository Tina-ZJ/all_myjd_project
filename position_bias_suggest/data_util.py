...
def load_preprocess(testfile):
    ...
    for i, line in enumerate(f):
        terms = line.strip().split('\t')
        id_pref = terms[0]+terms[1]
        if id_pref not in expo_id:
            if i!=0:
                features.append(features_tmp)
                lines.append(lines_tmp)
            lines_tmp = list()
            features_tmp = list()
            expo_id[id_pref]=0
        title = '\t'.join(terms[1:4])
        feature = json.loads(terms[-1])
        input_f = dict()
        input_f.setdefault('pref_prd_id',[feature['pref_prd_id']])
        input_f.setdefault('pref_cid3_id',[feature['pref_cid3_id']])
        input_f.setdefault('prefix_word_seg_id',[feature['pref_word_seg_id']])
        input_f.setdefault('sug_prd_id',[feature['sug_prd_id']])
        input_f.setdefault('suggest_cid',[feature['suggest_cid']])
        input_f.setdefault('suggest_word_seg_id',[feature['suggest_word_seg_id']])
        input_f.setdefault('gender_realt',[feature['gender_realt']])
        input_f.setdefault('cids_realt_long',[feature['cids_realt_long']])
        input_f.setdefault('srch_kwd_seg_id',[feature['srch_kwd_seg_id_long']])
        features_tmp.append(input_f)
        lines_tmp.append(title) 
    return lines, features
