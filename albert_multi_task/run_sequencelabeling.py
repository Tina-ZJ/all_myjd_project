...
flags.DEFINE_string(
        "bert_config_file", "./albert_tiny_zh/albert_config_tiny.json",
        "The config json file corresponding to the pre-trained BERT model. "
        "This specifies the model architecture.")

flags.DEFINE_string("task_name", "ner", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "./albert_tiny_zh/vocab.txt",
...
flags.DEFINE_string(
        "init_checkpoint", "./albert_tiny_zh/albert_model.ckpt",
...
flags.DEFINE_integer("predict_batch_size", 256, "Total batch size for predict.")
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, label_cid3=None):
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.label_cid3_id = label_cid3_id
...
class TestProcessor(DataProcessor):
    ...
    def get_labels_cid3(self):
        with codecs.open('data/labels_cid3.tsv', 'r', 'utf8') as fr:
            labels = []
            for line in fr:
                terms = line.strip().split('\t')
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label_cid3 = tokenization.convert_to_unicode(line[2])
            examples.append(
                    InputExample(guid=guid, text_a=text_a, text_b=None, label=label, label_cid3=label_cid3))
        return examples

def convert_single_example(ex_index, example, label_map, label_cid3_map,max_seq_length,
                                                     tokenizer):
    """Converts a single `InputExample` into a single `InputFeatures`."""
    text_list = example.text_a.split(' ')
    label_list = example.label.split(' ')
    label_cid3_list = example.label_cid3.split(',')
    tokens = []
    labels = []
    ...
    label_ids = []
    label_cid3_ids = []
    # covert one hot label
    for cid3 in label_cid3_list:
        label_cid3_ids.append(label_cid3_map[cid3])
    if len(label_cid3_ids) > max_seq_length:
        label_cid3_ids = label_cid3_ids[:max_seq_lenth]
    while len(label_cid3_ids) < max_seq_length:
        label_cid3_ids.append(int(-1))
    ...
    if ex_index < 3:
        ...
        tf.logging.info("label_cid3_ids: %s " % " ".join([str(x) for x in label_cid3_ids]))
...
def file_based_convert_examples_to_features(
        examples, label_map, label_cid3_map, max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, label_map, label_cid3_map,
        ...
        features["label_cid3_ids"] = create_int_feature(feature.label_cid3_id)
...
def file_based_input_fn_builder(num_cid3, input_file, seq_length, is_training,
                                                                drop_remainder):
    ...
    name_to_features = {
            ...
            "label_cid3_ids": tf.FixedLenFeature([seq_length], tf.int64),
    ...
    def _decode_record(record, name_to_features):
        ...
        for name in list(example.keys()):
            ...
            if t.dtype == tf.int64:
                t = tf.to_int32(t)
            example[name] = t
        #one_hot_enc = tf.one_hot(indices=example["label_cid3_ids"], depth=num_cid3)
        #example["label_cid3_ids"] = tf.reduce_sum(one_hot_enc, axis=0)  
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                                 labels, labels_cid3, num_labels, num_cid3_labels,use_one_hot_embeddings):
    ...
    loss, per_example_loss, logits, logits_cid3, acc, pred_label_ids, probabilities, predictions, precision, recall, f1 = inference(model, num_labels, num_cid3_labels, is_training, labels, labels_cid3, input_mask)

    return (loss, per_example_loss, logits, logits_cid3, pred_label_ids, probabilities, acc, predictions, precision, recall, f1)
...
def project_layer_cid3(inputs, num_labels):
    #logits = tf.layers.dense(inputs, num_labels, activation=None, kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))
    logits = tf.layers.dense(inputs, num_labels, activation=None)
    logits = tf.identity(logits, name='logits_cid3')
    return logits
...
def inference(model, num_labels, num_cid3_labels, is_training, labels, labels_cid3, input_mask, use_crf=True, use_rnn=False):
    output_layer = model.get_sequence_output()
    #output_layer = model.all_encoder_layers[0]
    #output_layer = model.get_embedding_output()
    # for multi label classification
    output_layer_first_token = model.get_pooled_output()
    #output_layer_first_token = tf.squeeze(output_layer[:, 0:1, :], axis=1)
    hidden_size = output_layer.shape[-1].value
    if is_training:
        keep_prob = 0.9
    else:
    ...
    logits_cid3 = project_layer_cid3(output_layer_first_token, num_cid3_labels)

    one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32)
    one_hot_labels_cid3 = tf.one_hot(labels_cid3, depth=num_cid3_labels, dtype=tf.float32)
    one_hot_labels_cid3 = tf.reduce_sum(one_hot_labels_cid3, axis=1)
    use_crf = False
    with tf.variable_scope("loss"):
        if use_crf:
        ...
        # for multi label class
        predictions = tf.sigmoid(logits_cid3, name='predictions')
        cid3_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=one_hot_labels_cid3, logits=logits_cid3)
        cid3_loss = tf.reduce_sum(cid3_loss, axis=1)
        cid3_loss = tf.reduce_mean(cid3_loss)
        tp = tf.reduce_sum(tf.cast(tf.greater(predictions, 0.3), tf.float32) * tf.cast(tf.equal(one_hot_labels_cid3, 1), tf.float32))    
        tn = tf.reduce_sum(tf.cast(tf.less(predictions, 0.3), tf.float32) * tf.cast(tf.equal(one_hot_labels_cid3, 0), tf.float32))
        fp = tf.reduce_sum(tf.cast(tf.greater(predictions, 0.3), tf.float32) * tf.cast(tf.equal(one_hot_labels_cid3,0), tf.float32))
        fn = tf.reduce_sum(tf.cast(tf.less(predictions, 0.3), tf.float32) * tf.cast(tf.equal(one_hot_labels_cid3,1), tf.float32))
    ...
    return (loss, per_example_loss, logits, logits_cid3, acc, pred_label_ids, probabilities, predictions, precision, recall, f1)
...
def model_fn_builder(bert_config, num_labels, num_cid3_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):    # pylint: disable=unused-argument
        ...
        label_cid3_ids = features["label_cid3_ids"]
        ...
        (total_loss, per_example_loss, logits, logits_cid3, pred_label_ids, probabilities, acc, predictions, precision, recall, f1) = create_model(
                bert_config, is_training, input_ids, input_mask, segment_ids, label_ids, label_cid3_ids,
                num_labels, num_cid3_labels, use_one_hot_embeddings)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                    ...
                    training_hooks=[tf.train.LoggingTensorHook(logged_tensors, every_n_iter=10)
                    ])
...
# This function is not used by this file but is still used by the Colab and
# people who depend on it.
def convert_examples_to_features(examples, label_list, label_cid3_map, max_seq_length,
                                                                 tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, label_list, label_cid3_map,
                                                                         max_seq_length, tokenizer)
...
def get_and_save_label_cid3_map(label_list, output_dir):
    label_map = {}
    with codecs.open(output_dir + '/label_cid3_map', 'w', 'utf8') as fw:
        for (i, label) in enumerate(label_list):
            label_map[label] = i
            fw.write(str(i) + '\t' + label + '\n')
    return label_map
...
def main(_):
    ...
    label_list = processor.get_labels()
    label_list_cid3 = processor.get_labels_cid3()
    label_map = get_and_save_label_map(label_list, FLAGS.output_dir)
    label_cid3_map = get_and_save_label_cid3_map(label_list_cid3, FLAGS.output_dir)


    model_fn = model_fn_builder(
            bert_config=bert_config,
            num_labels=len(label_list),
            num_cid3_labels=len(label_list_cid3),
    ...
    if FLAGS.do_train:
        ...
        if not train_file_exists: # if tf_record file not exist, convert from raw text file. # TODO
                file_based_convert_examples_to_features(train_examples, label_map, label_cid3_map,FLAGS.max_seq_length, tokenizer, train_file)
        tf.logging.info("***** Running training *****")
        tf.logging.info("    Num examples = %d", len(train_examples))
        tf.logging.info("    Batch size = %d", FLAGS.train_batch_size)
        tf.logging.info("    Num steps = %d", num_train_steps)
        train_input_fn = file_based_input_fn_builder(
                num_cid3=len(label_cid3_map),
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(
                eval_examples, label_map, label_cid3_map, FLAGS.max_seq_length, tokenizer, eval_file)
    ...
    if FLAGS.do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, label_map, label_cid3_map,
