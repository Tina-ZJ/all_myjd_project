...
BASE_DIR='/home/zhangjun386/mnt/zhangjun386/alBert/albert_multi_task'
...
tf.flags.DEFINE_float("threshold", 0.2, "threshold for predict")
...
def get_cid_name(cid3_file):
    ...
    with codecs.open(cid3_file, encoding='utf8') as f:
        for line in f:
            ...
            label_name[str(line_list[0])] = line_list[1]
    return label_name
...
def eval():
    ...
    for batch in batch_datas:
        ...
        continue 
        for  prediction, query, predictions_cid3 in zip(batch_predictions, querys, batch_predictions_cid3):
        ...
        f.write(''.join(query[1:-1])+'\t'+','.join(result)+'\t'+','.join(label_scores)+'\t'+','.join(label_names)+'\n')
