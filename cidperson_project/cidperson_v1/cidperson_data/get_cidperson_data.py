...
table_name='sz_algo_session_cid_samples'
...
def getSample(hiveCtx):
    sql="""
            select a.pvid as pvid, keyword, gender_realt, age_realt, search_keyword_seg_id, sku
            from 
            (
              select pvid,  keyword, int_feature['gender_realt'] as gender_realt, int_feature['age_realt'] as age_realt, int_feature['search_keyword_seg_id'] as search_keyword_seg_id 
              from app.user_query_feature_dump_szalgo where dt>='{yest_str}' and keyword!='' group by pvid, keyword, int_feature['gender_realt'], int_feature['age_realt'], int_feature['search_keyword_seg_id']
            )a
            join
            (
    ...
    sql_cid="""
            select  pvid, keyword, gender_realt, age_realt, search_keyword_seg_id, item_last_cate_cd as cidx, item_last_cate_name as cidx_name 
            from
            (
               select pvid, keyword, gender_realt, age_realt, search_keyword_seg_id, sku
               from tmp_table
            )a
            join
            (
            ...
            group by pvid, keyword, gender_realt, age_realt, search_keyword_seg_id, item_last_cate_cd, item_last_cate_name
        """.format(dt=yest_str)
