...
source_table = "sz_algo_session_cid_samples"
...
def map_par_fun_seg(rows):
    ...
    for r in rows:
        ...
        yield Row(pvid=r.pvid, keyword=keyword, segs=segs, gender_realt=r.gender_realt, age_realt=r.age_realt, search_keyword_seg_id=r.search_keyword_seg_id, cidx_list=r.cidx_list, cidx_name_list=r.cidx_name_list)


def run(sc, hiveCtx, dt_str, version):
    rdd = hiveCtx.sql("""
            select pvid, keyword, gender_realt, age_realt, search_keyword_seg_id, collect_set(cidx) as cidx_list, collect_set(cidx_name) as cidx_name_list from app.{source_table}
            where dt='{dt}' group by pvid, keyword, gender_realt, age_realt, search_keyword_seg_id """.format(dt=dt_str, source_table=source_table)).rdd.mapPartitions(map_par_fun_seg)
    ...
    def to_id(terms, genders, ages, sessions, _id_dict, max_v, padding=35):
        term_id_arr = [_id_dict[t] if t in _id_dict else 1 for t in terms]
        term_id_arr = term_id_arr[:8]
        for i in range(0, 8 - len(term_id_arr)):
            term_id_arr.append(0)


        cls = max_v+1
        sep = max_v+2
        genders = [max_v+x+3 for x in genders]
        ages = [max_v+x+7 for x in ages]
        input_ids = [cls]+term_id_arr+[sep]+sessions+[sep]+genders+[sep]+ages+[sep]
    ...
    def map_fun_toid(iters):
        ...
        cidx_name_dict = br_cidx_name_dict.value
        cidx_cid2_dict = br_cidx_cid2_dict.value
        cidx_cid1_dict = br_cidx_cid1_dict.value
        cid2_cidx_dict = br_cid2_cidx_dict.value
        cid1_cidx_dict = br_cid1_cidx_dict.value
        max_v = max(term_id_dict.values())
        for itr in iters:
            input_ids, input_mask, segment_ids = to_id(itr.segs, itr.gender_realt, itr.age_realt, itr.search_keyword_seg_id[:20], term_id_dict, max_v)
    ...
    #cidlist = rdd.flatMap(flat_fun_cid).distinct().collect()
    cid_sql = """
            select cidx 
            from
            (
            select cidx, count(1) as sum from app.sz_algo_session_cid_samples where dt='{dt}' group by cidx
            )a
            join
            (
    ...
    cid_sql = """
            ...
            """
    cidx_name_cid2_cid1 = hiveCtx.sql(cid_sql).rdd.collect()
    for x in cidx_name_cid2_cid1:
        if x[0] not in cidx_id_dict:
            continue
        cidx_name_dict.setdefault(x[0],x[1])
        cid2_cidx_dict.setdefault(x[2],[]).append(x[0])
        cid1_cidx_dict.setdefault(x[3],[]).append(x[0])
        cidx_cid2_dict.setdefault(x[0],x[2])
        cidx_cid1_dict.setdefault(x[0],x[3])
    ...
    for x in id_term:
        term_id_dict.setdefault(x[1],x[0])
...
#    term_id_dict.setdefault('[CLS]', max_id[0][0]+1)
#    term_id_dict.setdefault('[SEP]', max_id[0][0]+2)
#    term_id_dict.setdefault('[男性]', max_id[0][0]+3)
#    term_id_dict.setdefault('[女性]', max_id[0][0]+4)
#    term_id_dict.setdefault('[家庭]', max_id[0][0]+5)
#    term_id_dict.setdefault('[性别未知]', max_id[0][0]+6)
#    term_id_dict.setdefault('[年龄未知]', max_id[0][0]+7)
#    term_id_dict.setdefault('[25岁以下]', max_id[0][0]+8)
#    term_id_dict.setdefault('[26~35]', max_id[0][0]+9)
#    term_id_dict.setdefault('[36~45]', max_id[0][0]+10)
#    term_id_dict.setdefault('[46~55]', max_id[0][0]+11)
#    term_id_dict.setdefault('[55岁以上]', max_id[0][0]+12)
    ...
    hiveCtx.sql("""
        ...
        select idx, cidx
        from cidx_idx
    """.format(index_table=index_table, dt=dt_str, dim_type=version))

    # broadcast

    br_cidx_id_dict = sc.broadcast(cidx_id_dict)
    br_term_id_dict = sc.broadcast(term_id_dict)
    br_cidx_name_dict = sc.broadcast(cidx_name_dict)
    br_cidx_cid2_dict = sc.broadcast(cidx_cid2_dict)
    br_cidx_cid1_dict = sc.broadcast(cidx_cid1_dict)
    br_cid2_cidx_dict = sc.broadcast(cid2_cidx_dict)
    br_cid1_cidx_dict = sc.broadcast(cid1_cidx_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-v", "--input_version", help="input version", default="v1")
