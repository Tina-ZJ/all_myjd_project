...
source_table2='app_algo_term_weight_groudtruth'
save_table='sz_algo_sku_term_weights_groudtruth'
...
def create_table(hiveCtx):
    ...
    create_tbl = """
        CREATE EXTERNAL TABLE IF NOT EXISTS app.{table_name} (
            ...
            features_combine                      array<float>               COMMENT   '合并后的特征'
            )
...
def score_func(rows):
    for row in rows:
        ...
        sku, terms, cid3, cid3_name, term_list, brand_list, ctf_list, confidence_list, rf_list, icf_list, igm_list = row
        score_list = list()
        ...
        terms = terms.split(',')
        for t in terms:
            f = terms_f.get(t, [0.0]*6)
            terms_l.extend(f)
        yield sku, terms, cid3, cid3_name, terms_f, terms_l
...
######################### get sku cid3 label #############################
    ...
    sql = """
                select sku_id as sku, sku_segs as terms, cid3, cid3_name
                from
                (
                    select sku_id, sku_segs
                    from {source_table2}
                    where dt='{dt}'
                )a
                join
                ...
                on a.sku_id=b.item_sku_id
        """.format(source_table2=source_table2, dt='active')
    ...
    sql="""
        select sku, terms, a.cid3 as cid3, cid3_name, a.term as term, brand_flag, ctf, ctf_confidence, rf, icf, igm
        from
        (
            select sku, terms, cid3, cid3_name, term 
            from table_sku
            LATERAL VIEW explode(split(terms,',')) t as term
        )a
        join
        (
    ...
    sql="""
        select sku, terms, cid3, cid3_name, collect_list(term) as term_list, collect_list(brand_flag) as brand_list, collect_list(ctf) as ctf_list, 
        ...
        from table_term
        group by sku, terms, cid3, cid3_name
        """

    print(sql)
    rdd = hiveCtx.sql(sql).rdd.cache()
    rdd.repartition(1000).mapPartitions(lambda rows: score_func(rows))\
        .toDF(['sku', 'terms', 'cid3', 'cid3_name', 'term_features', 'features_combine']).registerTempTable("score_table")
