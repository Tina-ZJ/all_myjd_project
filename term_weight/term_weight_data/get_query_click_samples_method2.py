...
def create_table(hiveCtx):
    ...
    create_tbl = """
        CREATE EXTERNAL TABLE IF NOT EXISTS app.{table_name} (
            ...
            agg_querys                            array<array<string>>              COMMENT   '聚合的query/sku集合',
            terms                                 array<string>              COMMENT       'query分词',
            avg_weights                          array<float>                   COMMENT       'term词权重'
            )
...
def score_func(rows):
    for row in rows:
        terms_pv = dict()
        query, terms, sku_seg_list, pv_list = row

        # 统计每个term的词频
        for sku_seg, pv in zip(sku_seg_list, pv_list):
            for x in sku_seg:
        ...
        # top 10 
        #sku_segs = [','.join(segs) for segs in sku_seg_list[:10]]
        yield query, len(sku_seg_list), sku_seg_list[:10], terms, weight_combine
...
######################### get query cid3 label #############################
    ...
    # get sku name
    sql="""
        select key_word, terms, sku_name_qp_seg as sku_seg, pv
        from
        (
            select sku, key_word, terms, pv
            from table_term
        ...
        join
        (
            select item_sku_id, sku_name_qp_seg from app.app_m03_search_item_sku_da_qp
            where dt='{dt}' and dim_type='base'
        )b
    ...
    sql="""
        select key_word, terms, collect_list(sku_seg) as sku_seg_list, collect_list(pv) as pv_list
        from table_sku
    ...
    rdd.repartition(500).mapPartitions(lambda rows: score_func(rows))\
        .toDF(['query','agg_len','agg_querys','terms','weights']).registerTempTable("score_table")
