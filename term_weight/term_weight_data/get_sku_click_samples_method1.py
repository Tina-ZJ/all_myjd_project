...
def score_func(rows):
    for row in rows:
        ...
        if score_sum < 0.000001:
            continue
        weight_list = [x/score_sum for x in weight_list]
        weight_list = [t+':'+str(x) for t,x in zip(sku_seg, weight_list)]

        yield sku, len(query_seg_list), query_seg_list[:10], sku_seg, weight_list
...
######################### get query cid3 label #############################
    ...
    sql="""
        select sku, key_word, sku_name_qp_seg as sku_seg, pv
        from
        (
        ...
        join
        (
            select item_sku_id, sku_name_qp_seg 
