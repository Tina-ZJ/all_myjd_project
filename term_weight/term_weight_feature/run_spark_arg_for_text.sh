...
cd submd
wget "http://storage.jd.local/query-suggest/git_code/QueryRootCommonModule.zip?Expires=3779002991&AccessKey=TA75Nt4TQ4RX7cLc&Signature=wr82jOIjqOzJbe4NJ4t%2FckAV21k%3D" -O QueryRootCommonModule.zip
unzip QueryRootCommonModule.zip -d QueryRootCommonModule
rm QueryRootCommonModule.zip 
...
--files $HIVE_CONF_DIR/hive-site.xml,${SUGGEST_DBPATH}/qp_common_file/correct_pair_black,${SUGGEST_DBPATH}/qp_common_file/correct_pair_white,${SUGGEST_DBPATH}/qp_common_file/mashup.dat.v2.0,${norm_file} \
...
--py-files ${PyFilePath}/QueryRootCommonModule.zip,${PyFilePath}/settings.zip,${PyFilePath}/QueryRootCommonModule/thirdparty/lib/esm.so,${seg_so},${norm_so},${workspace}/src/qp_apps/session_adl/event_param_parser.py,${workspace}/src/qp_apps/session_adl/event_param_setting.py,${workspace}/src/qp_apps/applications/muti_task/conf.py,${workspace}/src/qp_apps/session_adl/event_param_setting.py,${workspace}/src/qp_apps/applications/scene_task/common_scene/scene_conf.py \
$@
